...
def transform(files, **kwargs):    
    ...
    socDict = CodeList_Codes_and_Labels('soc')
    def SocLabels(value):
        return socDict[value]
    ...
    for i in range(0, numberOfIterations):
        ...
        for tab in tabs:
            ...
            ageSocGroups = tab.excel_ref('A' + Min + ':A' + Max).is_not_blank().is_not_whitespace() - junk
            
            socCodes = ageSocGroups.shift(1, 0)
            ...
            obs = variable.waffle(ageSocGroups)
            
            dimensions = [
                    HDimConst(TIME, year_of_data),
                    HDimConst(GEOG, geog),
                    HDim(ageSocGroups, 'ageSocGroups', DIRECTLY, LEFT),
    ...
    df['ageSocGroups'] = df['ageSocGroups'].apply(lambda x: x.strip())
    
    df['AgeGroups'] = df['ageSocGroups'].apply(AgeGroups)
    df['age-groups'] = df['AgeGroups'].apply(lambda x: x.lower().replace(' ', '-'))
    ...
    df = df[[
            'OBS', 'DATAMARKER', 'CV', 'Time_codelist', 'TIME', 'GEOG', 'Geography',
            'socCodes_codelist', 'socCodes', 'Variable_codelist', 'Variable', 
            'sheetName_codelist', 'sheetName', 'age-groups', 'AgeGroups', 'sex', 'Sex',
